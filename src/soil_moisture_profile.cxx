...
void soil_moisture_profile::SoilMoistureProfile(
    ...
    parameters->shape[0] =
        parameters->ncells; // this is used for the size of soil_moisture_profile (bmi output)
...
/*
  ...
  chosen; option include `constant` or `linear`. The option `constant` assigns a constant value to
  discretized cells within each layer, `linear` option linearly interpolate values between layers
  and interpolated values are assigned to the soil discretization
  @params input_var_names_model (1D)      : dynamically sets model inputs to be used in the bmi
  input_var_names
  ...
  @param soil_moisture_fraction_depth [m] : user specified depth for the fraction of soil moisture
  (default is 40 cm)
...
void soil_moisture_profile::InitFromConfigFile(
    ...
    while (fp) {
        ...
        // NOTE: `soil_params.satpsi` may be deprecated in the future in favor of `satpsi`
            ...
            else if (param_value == "topmodel" || param_value == "TopModel" ||
                     param_value == "TOPMODEL")
    ...
    if (parameters->soil_storage_model == Topmodel) {
        ...
        if (is_water_table_based_method_set) {
            ...
            stringstream errMsg;
            errMsg
                << "soil_storage_model is set to Topmodel in the config file " << config_file
...
void soil_moisture_profile::SoilMoistureProfileUpdate(struct soil_profile_parameters* parameters) {
    ...
    parameters->soil_moisture_fraction =
        fmin(parameters->soil_moisture_fraction, parameters->soil_storage);
...
/*
  ...
  @param tolerance             [cm] : Error tolerance for finding the new root (i.e.,
  water_table_depth)
...
void soil_moisture_profile::SoilMoistureProfileFromConceptualReservoir(
    ...
    if (soil_storage_change_per_timestep_cm > 0.0001 || parameters->init_profile) {
        ...
        do {
            ...
            double fis = parameters->smcmax[0] * (zi - z0) + parameters->smcmax[0] * satpsi_cm +
                         alpha * parameters->smcmax[0] *
                         (pow((model_depth - zi), beta) - pow(satpsi_cm, beta));
            double fib = parameters->smcmax[0] * (zi - z0) + parameters->smcmax[0] * satpsi_cm +
                         alpha * parameters->smcmax[0] * 
                         (pow(abs(zb - zi), beta) - pow(satpsi_cm, beta));
    ...
    if (verbosity.compare("high") == 0) {
        ...
        std::cout << "Given soil water = " << parameters->soil_storage
                  << ", Computed soil water = " << total_water << "\n";
    }
...
/*
  ...
  - Note: the water table location is the thickness of the water table plus saturated capillary head
  (satpsi)
  - local_variables:
  @param lam                [-]  : 1/b (b: pore size distribution)
...
void soil_moisture_profile::SoilMoistureProfileFromLayeredReservoir(
    ...
    if (verbosity.compare("high") == 0) {
        ...
        for (int i = 0; i < num_wf; i++)
            std::cerr << "SoilMoistureProfile (input): (depth, water_content) = "
...
void soil_moisture_profile::FindWaterTableLayeredReservoir(
    ...
    if (fabs(
            parameters->soil_moisture_wetting_fronts[num_wf - 1] -
...
void soil_moisture_profile::SoilMoistureProfileFromWaterTableDepth(
    ...
    if (parameters->water_table_based_method == Deficit_based || parameters->init_profile) {
        parameters->water_table_depth = parameters->global_deficit / delta_theta * to_cm +
        ...
        parameters->water_table_depth -= (parameters->Qv_topmodel - parameters->Qb_topmodel) /
                                          parameters->cat_area * dt * to_cm;
    }
...
void soil_moisture_profile::PrintSoilMoistureProfile(struct soil_profile_parameters* parameters) {
    for (int i = 0; i < parameters->ncells; i++)
        std::cout << "soil_moisture (z, value) = " << parameters->soil_z[i] << ", "
