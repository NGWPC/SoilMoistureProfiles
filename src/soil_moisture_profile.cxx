...
/*
  ...
  @input soil_moisture_profile_option (string) : valid only when layered soil reservoir model is
                                                 chosen; option include `constant` or `linear`. 
                                                 The option `constant` assigns a constant value to
                                                 discretized cells within each layer, `linear` option 
                                                 linearly interpolate values between layers
                                                 and interpolated values are assigned to the soil discretization
  @params input_var_names_model (1D)      : dynamically sets model inputs to be used in the bmi input_var_names
  @input water_table_depth (double)       : water table depth, default is 6 m for layered model,
                                            conceptual model computes its own
  @param soil_moisture_fraction     [-]   : fraction of soil moisture in the top 40 cm or
                                            user-defined soil_moisture_fraction_depth (water in the topsoil over total water)
  ...
  @param water_table_based_method    (string) : Flux_based (for iteration-based water table)
                                                Deficit_based (for deficit-based water table) and
...
void soil_moisture_profile::InitFromConfigFile(
    ...
    while (fp) {
        ...
        if (param_key == "soil_z") {
            ...
            if (!(parameters->satpsi > 0.0)) {
                LOG(LogLevel::FATAL, "Parameter %s=%f must be > 0.0", param_key.c_str(), parameters->satpsi);
                throw std::runtime_error("Bad satpsi value");
            }
            is_satpsi_set = true;
            continue;
        }
    ...
    if (!is_soil_storage_model_set) {
        stringstream errMsg;
        errMsg << "soil_storage_model not set in the config file " << config_file
               << ". Options 'conceptual or layered' \n";
        LOG(LogLevel::FATAL, errMsg.str());
        throw runtime_error(errMsg.str());
    }
    ...
    if (parameters->soil_storage_model == Layered) {
        if (!is_soil_moisture_profile_option_set) {
            stringstream errMsg;
            errMsg << "soil_moisture_profile_option_set key is not set in the config file "
                   << config_file << ", options = constant or linear \n";
            LOG(LogLevel::FATAL, errMsg.str());
            throw runtime_error(errMsg.str());
        }
    ...
    if (parameters->soil_storage_model == Topmodel) {
        if (is_water_table_based_method_set) {
            if (parameters->water_table_based_method != Flux_based &&
                parameters->water_table_based_method != Deficit_based) {
                stringstream errMsg;
                errMsg << "water_table_based_method key is set in the config file " << config_file
                       << " with wrong options. Options = flux_based or deficit_based \n";
                LOG(LogLevel::FATAL, errMsg.str());
                throw runtime_error(errMsg.str());
            }
        }
...
void soil_moisture_profile::SoilMoistureProfileUpdate(struct soil_profile_parameters* parameters) {
    ...
    // update the soil storage for the top 2 m to be consistent with NWM; needed only when coupled
    // to Topmodel
    ...
    for (int i = 0; i < parameters->ncells; i++) {
        if (parameters->soil_z[i] <= soil_moisture_fraction_depth && i == 0) {
            parameters->soil_moisture_fraction +=
                parameters->soil_moisture_profile[i] * parameters->soil_z[i];
...
/*
  ...
  @param z2  [cm]                     : depth of the water table at the current timestep (unknown, will use the
  ...
  @param soil_storage_previous_timestepcm [cm] : soil storage at the previous timestep computed
                                                 locally from the watertable location at the
                                                 previous timestep
...
void soil_moisture_profile::SoilMoistureProfileFromConceptualReservoir(
    struct soil_profile_parameters* parameters
) {
    ...
    double beta        = 1.0 - lam;
    double alpha       = pow(satpsi_cm, lam) /
                         beta; // a constant term obtained in the integration of the soil moisture function
    ...
    if (soil_storage_change_per_timestep_cm > 0.0001 || parameters->init_profile) {
        ...
        // check if the storage is greater than the maximum soil storage. if yes, set it to the
        // maximum storage
        ...
        // this won't happen, hopefully, if it does then assign a small non-zero number to the soil
        // moisture profile also to avoid computing water table and over-estimated soil moisture for
        // extremely dry soils
        ...
        // note for stability reasons, the loop is terminated when the water-table depth exceeds
        // 1000m, that means the soil is super dry
        do {
            count++;
            ...
            // fis is integrates the function from z0 to the surface: 1st part: saturated soil
            // between zi and z0; 2nd part: capillary fringe; 3rd: enclosed area between satpis and
            // the surface fib is non-zero for zi <0, zero otherwise. this is the volume of water
            // that needs to be subtracted from "fis" to get the water storage in the computational
            // domain (say top 2 m if soil column has depth 2m)
            ...
            double dfis = parameters->smcmax[0] - alpha * parameters->smcmax[0] * beta *
                          pow((model_depth - zi), beta - 1.0);
            double dfib = parameters->smcmax[0] -
                          alpha * parameters->smcmax[0] * beta * pow(fabs(zb - zi), beta - 1.0);
            ...
            // if the water gets below 1000 m, that would mean the soil is super dry and the
            // algorithm may fail to converge in reasonable number of timesteps
            if (zi_m < -1000)
                break;

        } while (fabs(diff) > tolerance);

        // water table thickness can be negative and that would be depth of the water table below
        // the depth of the computational domain; probably a better name would be
        // water_table_location
    ...
    if (GetLogLevel() == LogLevel::DEBUG) {
        ...
        for (int i = 1; i < parameters->ncells; i++) {
            total_water += parameters->soil_moisture_profile[i] *
                           (parameters->soil_z[i] - parameters->soil_z[i - 1]);
        }
        LOG(LogLevel::DEBUG,
            "Given soil water =  %f, Given soil water =  %f ",
    ...
    for (int i = 0; i < parameters->ncells; i++) {
        if (!(parameters->soil_moisture_profile[i] <= parameters->smcmax[0]) ||
            ...
            throw std::runtime_error("Bad soil_moisture_profile value");
        }
    }
}

/*
  - Computes 1D soil moisture profile for layered/calculated reservoir. That is, take layered
    reservior soil moisture and distribute it vertically to the desired soil discretization
  - Two strategies are implemented
    - Constant strategy: A simple technique to map layered values to grids in the soil discretization.
                         That is, all grid cells in the discretization has a constant value within a
                         layer. Note cells at the interface take average value of the layers
    - Linear strategy: A linear interpolation tehcnique is used to map layered values to grids in the
                       soil discretization. That is, grid cells in the discretization take linearly
                       interpolated value between consecutive layers.
...
void soil_moisture_profile::SoilMoistureProfileFromLayeredReservoir(
    ...
    if (parameters == nullptr) {
        LOG(LogLevel::FATAL, "SoilMoistureProfileFromLayeredReservoir received null parameters");
        throw std::runtime_error("Null parameters in SoilMoistureProfileFromLayeredReservoir");
    }
    ...
    if (num_wf <= 0) {
        LOG(LogLevel::FATAL, "Invalid num_wetting_fronts=%d in SoilMoistureProfileFromLayeredReservoir", num_wf);
        throw std::runtime_error("Invalid num_wetting_fronts in SoilMoistureProfileFromLayeredReservoir");
    }

    if ((int)parameters->soil_depth_wetting_fronts.size() < num_wf) {
        LOG(LogLevel::FATAL,
            "soil_depth_wetting_fronts size=%zu is smaller than num_wf=%d in SoilMoistureProfileFromLayeredReservoir",
            parameters->soil_depth_wetting_fronts.size(), num_wf);
        throw std::runtime_error("soil_depth_wetting_fronts smaller than num_wf");
    }

    if ((int)parameters->soil_moisture_wetting_fronts.size() < num_wf) {
        LOG(LogLevel::FATAL,
            "soil_moisture_wetting_fronts size=%zu is smaller than num_wf=%d in SoilMoistureProfileFromLayeredReservoir",
            parameters->soil_moisture_wetting_fronts.size(), num_wf);
        throw std::runtime_error("soil_moisture_wetting_fronts smaller than num_wf");
    }

    if (parameters->soil_moisture_profile == nullptr) {
        LOG(LogLevel::FATAL, "soil_moisture_profile is null in SoilMoistureProfileFromLayeredReservoir");
        throw std::runtime_error("Null soil_moisture_profile in SoilMoistureProfileFromLayeredReservoir");
    }

    if (parameters->soil_z == nullptr) {
        LOG(LogLevel::FATAL, "soil_z is null in SoilMoistureProfileFromLayeredReservoir");
    ...
    for (int i = 0; i < num_wf; i++) {
        if (!std::isfinite(parameters->soil_depth_wetting_fronts[i]) ||
            ...
            LOG(LogLevel::FATAL,
                "Invalid wetting front input at i=%d: depth=%e moisture=%e",
                i,
                parameters->soil_depth_wetting_fronts[i],
                parameters->soil_moisture_wetting_fronts[i]);
            throw std::runtime_error("Invalid wetting front input in SoilMoistureProfileFromLayeredReservoir");
        }
    }
    ...
    if (parameters->soil_moisture_profile_option == Constant) {
        ...
        for (int i = 0; i < parameters->ncells; i++) {
            ...
            if (parameters->soil_z[i] <= parameters->last_layer_depth) {
                ...
                if (i == 0 && c + 1 < num_wf &&
                    parameters->soil_z[i] > parameters->soil_depth_wetting_fronts[c]) {
                    ...
                    else if (parameters->soil_z[i - 1] == parameters->soil_depth_wetting_fronts[c] &&
        ...
        for (int i = 0; i < parameters->ncells; i++) {
            if (parameters->soil_z[i] <= parameters->soil_depth_wetting_fronts[0]) {
                ...
                if (i + 1 < parameters->ncells &&
                    parameters->soil_z[i + 1] > parameters->soil_depth_wetting_fronts[0])
                    c++;
            }
            // linear interpolation between consecutive layers
                ...
                if (i + 1 < parameters->ncells &&
                    parameters->soil_z[i + 1] > parameters->soil_depth_wetting_fronts[c])
                    c++;
            } else {
                // extend the profile below the last layer depth, covering some corner cases
                double theta;
...
void soil_moisture_profile::SoilMoistureProfileFromWaterTableDepth(
    struct soil_profile_parameters* parameters
) {
    // converting variables to cm for numerical reasons only
    double satpsi_cm      = parameters->satpsi * 100.;
    ...
    if (parameters->water_table_based_method == Deficit_based || parameters->init_profile) {
        parameters->water_table_depth =
            parameters->global_deficit / delta_theta * to_cm +
            satpsi_cm; // add saturated head to account for capillary fringe
        ...
        parameters->water_table_depth -=
            (parameters->Qv_topmodel - parameters->Qb_topmodel) / parameters->cat_area * dt * to_cm;
    }

    // check if the storage is greater than the maximum soil storage. if yes, set it to the maximum
    // storage
    if (parameters->water_table_depth == 0.0) {
        for (int j = 0; j < parameters->ncells; j++)
            parameters->soil_moisture_profile[j] = parameters->smcmax[0];
    ...
    double dz_v = (model_depth - zi - satpsi_cm) /
                  z_hres; // vertical spacing over the depth (from zi+satpsi to the surface)
    ...
    // map the high resolution soil moisture curve to the soil discretization depth that is provided
    // in the config file
    for (int i = 0; i < parameters->ncells; i++) {
...
vector<double> soil_moisture_profile::ReadVectorData(string param_name, string param_value) {
    int pos          = 0;
    string delimiter = ",";
    ...
    if (z1.find(delimiter) == string::npos) {
        ...
        if (v <= 0.0) {
            stringstream errMsg;
            errMsg << "Input provided in the config file for parameter " << param_name << " is "
                   << v << ". It should be positive." << "\n";
            LOG(LogLevel::FATAL, errMsg.str());
            throw runtime_error(errMsg.str());
        }
...
void soil_moisture_profile::PrintSoilMoistureProfile(struct soil_profile_parameters* parameters) {
    for (int i = 0; i < parameters->ncells; i++)
        LOG(LogLevel::DEBUG,
            "soil_moisture (z, value) = %f, %f ",
            parameters->soil_z[i],
            parameters->soil_moisture_profile[i]);
}
